...
Each row also has a NETWORK link (2026-08-12): it opens a citation-context
graph of that paper in a new tab -- the paper's references and citing works
as circles (size = citation count, colour = publication year), linked by
shared references (bibliographic coupling, the classic open bibliometric
method; direct citations weigh extra). The graph lives in a static
`network.html` written next to every results page; it fetches its data LIVE
from the open OpenAlex API (CC0) only when opened, so viewing a graph needs
internet while the results page itself stays fully offline-readable. Only
the paper's DOI or title is sent -- never paper content -- and no language
model is involved anywhere; the page discloses its method and that citation
coverage is incomplete for some publishers. arXiv-only records resolve by
title search (OpenAlex does not index arXiv DataCite DOIs), disclosed on
...
local file:// page cannot write files into your library; it only
