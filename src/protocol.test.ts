...
{
	assert.equal(protocolLogPath("/root", "2026-07-16", "a"), "/root/lit-synthesis/protocols/2026-07-16_a.json");
	assert.equal(protocolLogPath("/root", "2026-07-16", "a", 2), "/root/lit-synthesis/protocols/2026-07-16_a_2.json");
	assert.equal(protocolLogPath("/root", "2026-07-16", "arxiv_2401.16393", 5), "/root/lit-synthesis/protocols/2026-07-16_arxiv_2401.16393_5.json");
}
...
/* ---------------- appendRound: create, append, schema ---------------- */
{
	...
	assert.equal(first.path, "/lit-synthesis/protocols/2026-07-16_a.json");
...
}
{
	...
	legacy.schema = 1;
	files.set("/lit-synthesis/protocols/2026-07-16_a.json", JSON.stringify(legacy));
	const appended = appendRound("/", paperA, makeRound("neu"), deps, () => {});
	assert.equal(appended.path, "/lit-synthesis/protocols/2026-07-16_a.json"); // NOT quarantined
...
/* ---------------- appendRound: quarantine, never overwrite ---------------- */
{
	...
	const { deps, files } = memoryDeps();
	files.set("/lit-synthesis/protocols/2026-07-16_a.json", "{ not json");
	const warnings: string[] = [];
	const appended = appendRound("/", paperA, makeRound("q"), deps, (m) => warnings.push(m));
	assert.equal(appended.path, "/lit-synthesis/protocols/2026-07-16_a_2.json");
	assert.equal(appended.roundNumber, 1);
	assert.equal(files.get("/lit-synthesis/protocols/2026-07-16_a.json"), "{ not json"); // untouched
	assert.ok(warnings.some((m) => m.includes("not readable")));
}
{
	// Same path, different paper identity (renamed PDF): also quarantined.
	const { deps, files } = memoryDeps();
	files.set("/lit-synthesis/protocols/2026-07-16_a.json", protocolOf("2026-07-16", [makeRound("old")], "doi:10.9999/other"));
	const warnings: string[] = [];
	const appended = appendRound("/", paperA, makeRound("new"), deps, (m) => warnings.push(m));
	assert.equal(appended.path, "/lit-synthesis/protocols/2026-07-16_a_2.json");
	assert.equal((JSON.parse(files.get("/lit-synthesis/protocols/2026-07-16_a.json")!) as Protocol).rounds[0].question, "old");
...
/* ---------------- loadRounds: anchored matching, one session ---------------- */
{
	...
	files.set("/lit-synthesis/protocols/2026-07-15_a.json", protocolOf("2026-07-15", [makeRound("Dienstag", "s1"), makeRound("fruehere Session", "s0")]));
	files.set("/lit-synthesis/protocols/2026-07-16_a.json", protocolOf("2026-07-16", [makeRound("Mittwoch 1", "s1"), makeRound("Mittwoch 2", "s1"), makeRound("Altbestand ohne Session")]));
	files.set("/lit-synthesis/protocols/2026-07-16_a_2.json", protocolOf("2026-07-16", [makeRound("Quarantaene-Nachfolger", "s1")]));
	files.set("/lit-synthesis/protocols/2026-07-12_a.json", protocolOf("2026-07-12", [makeRound("nur fremde Session", "s0")]));
	files.set("/lit-synthesis/protocols/2026-07-14_a.json", "{ corrupt");
	files.set("/lit-synthesis/protocols/2026-07-13_a.json", protocolOf("2026-07-13", [makeRound("fremd", "s1")], "doi:10.9999/other"));
	files.set("/lit-synthesis/protocols/2026-07-16_ab.json", protocolOf("2026-07-16", [makeRound("anderes Paper", "s1")])); // base "ab" != "a"
	files.set("/lit-synthesis/protocols/2026-07-16_Paper_chat_report_a.json", "{}"); // report sidecar, never ingested
	const warnings: string[] = [];
	...
	assert.deepEqual(used, [
		"/lit-synthesis/protocols/2026-07-15_a.json",
		"/lit-synthesis/protocols/2026-07-16_a.json",
		"/lit-synthesis/protocols/2026-07-16_a_2.json",
	]);
	...
	dotted.files.set("/lit-synthesis/protocols/2026-07-16_arxiv_2401x16393.json", protocolOf("2026-07-16", [makeRound("Falle", "s1")]));
...
/* ---------------- sticky scope: session semantics ---------------- */
{
	...
	files.set("/lit-synthesis/protocols/current-scope.json", JSON.stringify({ papers: [], session: "s1" }));
